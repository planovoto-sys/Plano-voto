...
declare
  ...
  groups jsonb;
  choices jsonb;
  shared_election text;
begin
  if principal is null then raise exception 'AUTH_REQUIRED'; end if;
  -- Mesma ordem dos salvamentos/exclusões: eleição antes da publicação.
  select election_id into shared_election from private.shared_selections where id = p_id and active;
  if not found then raise exception 'SHARE_UNAVAILABLE'; end if;
  perform pg_advisory_xact_lock(hashtextextended('recommendations:' || shared_election, 0));
  select * into shared from private.shared_selections where id = p_id and active for share;
  if not found then raise exception 'SHARE_UNAVAILABLE'; end if;
  if shared.revision is distinct from p_revision then raise exception 'SHARE_CHANGED'; end if;
...
-- now() é o início da transação: duas gravações na mesma transação podiam
-- manter a mesma versão e aceitar uma confirmação já desatualizada.
create or replace function private.set_ballot_draft_updated_at()
...
$$;
revoke all on function private.set_ballot_draft_updated_at() from public, anon, authenticated;
drop trigger ballot_drafts_set_updated_at on public.ballot_drafts;
create trigger ballot_drafts_set_updated_at before update on public.ballot_drafts
  for each row execute function private.set_ballot_draft_updated_at();
