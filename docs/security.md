...
Este projeto ainda usa Firebase Auth e Firestore nos fluxos em producao e prepara o Supabase como backend alvo. Ambos seguem negacao por padrao. As chaves Web do Firebase e a publishable key do Supabase sao identificadores publicos necessarios no navegador; nenhuma concede acesso administrativo. A protecao real usa Auth, IAM/Rules ou RLS, validacao no servidor e rate limit.
...
6. No Supabase, exponha ao navegador apenas `VITE_SUPABASE_URL` e `VITE_SUPABASE_PUBLISHABLE_KEY`. Mantenha `SUPABASE_SERVICE_ROLE_KEY`, URL/senha do banco e `SUPABASE_ACCESS_TOKEN` somente em ambientes protegidos do backend ou CI.
...
| 3 | Chave pública/DB | Chaves Web/publishable sao tratadas como publicas. Firebase depende de Rules/Auth/IAM; Supabase depende de grants, RLS e Auth. |
| 4 | RLS | Firestore Rules seguem negacao por padrao. No Supabase, todas as tabelas `public` possuem RLS e grants explicitos; tabelas internas ficam no schema `private`. |
...
| 18 | Headers | CSP, HSTS, `nosniff`, anti-frame, Referrer Policy, Permissions Policy, COOP e CORP estao configurados para Firebase, Supabase e Vercel. |
