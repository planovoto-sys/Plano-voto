...
export default function Home() {
  ...
  const executarMudanca = async (novoEstado) => {
    ...
    try {
      const draftAnterior = user?.uid
        ? readBallotDraft(user.uid)
        : readVisitorBallotDraft();
      const estadoAtual = draftAnterior.estado || estadoSelecionado || null;
      ...
      if (estadoAtual && estadoAtual !== novoEstado) {
        const previousCandidateIds = getDraftActiveCandidateIds(draftAnterior);
        invalidateCandidateTalliesCache(previousCandidateIds, { estado: estadoAtual });
        invalidateStateChoiceCountsCache([estadoAtual, novoEstado]);
      }
